...
function initGame() {
	...
	intervalId = setInterval(function () {
		...
		if (!gBoard[randomI][randomJ].gameElement) {
...
function buildBoard() {
	...
	for (var i = 0; i < board.length; i++) {
		for (var j = 0; j < board[0].length; j++) {
			...
			if (i === 0 && j === Math.floor(board[0].length / 2 ) ||
				i === board.length - 1 && j === Math.floor(board[0].length / 2) ||
				i === Math.floor((board.length - 1) / 2) && j === 0 ||
				i === Math.floor((board.length - 1) / 2) && j === board[0].length - 1) {
				cell.type = FLOOR;
			}
...
function moveTo(i, j) {
	...
	if ((iAbsDiff === 1 && jAbsDiff === 0) || (jAbsDiff === 1 && iAbsDiff === 0)) {
		...
		if (targetCell.gameElement === BALL) {
			...
			audio.play();
			count++;
			console.log(`You collected ${count} balls`);
		}
